...
while [ $SECONDS -lt $end ]; do
    ...
    cd ../../scripts
    python3 gmutate.py ../${LANG}/${GFILE_P} mutant-run-${RUN}-${SUT} ${S} ../${LANG}/${GFILE_L}

    cd ../${LANG}/${TOOL}/mutant-run-${RUN}-${SUT}
    grammarinator-process ${GFILE_P} ${GFILE_L} &> /dev/null
    cd ..
    fi
    cd mutant-run-${RUN}-${SUT}

    grammarinator-generate XMLGenerator.XMLGenerator -r document -o ../tests/input_${RUN}_${SUT}.in -d 60 -n 1 --random-seed ${S} --sys-path . -j=1 -s grammarinator.runtime.simple_space_serializer &> /dev/null
    EXIT_CODE=$?
